  ...
  return new Promise(function (resolve) {
    ...
    if (arg.length === 2) {
      if (arg[1] === '--stats') {
        ...
        resolve(" Total:  ".concat((0, _stats.funcionStats)(_links).total, "\n unique: ").concat((0, _stats.funcionStats)(_links).unicos));
      }

      if (arg[1] === '--validate') {
        (0, _validate.default)(arg[0]).then(function (response) {
          return response.forEach(function (ele) {
            return resolve("".concat(ele.ruta, " ").concat(ele.link, " ").concat(ele.texto, " ").concat(ele.status, " ").concat(ele.statusText));
          });
        });
      }
    }
    ...
    if (arg.length === 1) {
      (0, _joinLink.default)(arg[0]).forEach(function (ele) {
        return resolve("".concat(ele.ruta, " ").concat(ele.link, " ").concat(ele.texto));
      });
    }
  });
};
